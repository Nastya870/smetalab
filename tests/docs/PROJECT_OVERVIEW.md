# 📋 СПРАВКА ПО ПРОЕКТУ SMETA-LAB

## 🎯 ОПИСАНИЕ ПРОЕКТА

**SMETA-LAB** - веб-приложение для управления строительными сметами, проектами и закупками с поддержкой мультитенантности (multi-tenant SaaS).

**Основные возможности:**
- Создание и управление строительными сметами
- Управление проектами и командами
- Справочники работ и материалов
- Управление закупками и поставщиками
- Генерация актов выполненных работ (КС-2, КС-3)
- Формирование договоров и графиков работ
- Ролевая система доступа (RBAC)
- Мультитенантная архитектура (каждая компания изолирована)

---

## 🛠️ ТЕХНОЛОГИЧЕСКИЙ СТЕК

### **Frontend**

**Фреймворк и библиотеки:**
- **React 18.3.1** - основной UI фреймворк
- **Vite 6.0.7** - сборщик и dev-сервер (порт 5173)
- **React Router DOM 7.0.2** - маршрутизация
- **Material-UI (MUI) 6.2.0** - UI компоненты
- **Redux Toolkit 2.3.0** - управление состоянием
- **React Hook Form 7.54.0** - работа с формами
- **Axios 1.7.9** - HTTP клиент

**Дополнительные библиотеки:**
- **Recharts 2.15.0** - графики и диаграммы
- **date-fns 4.1.0** - работа с датами
- **react-to-print 3.0.4** - печать документов
- **docxtemplater 3.57.1** - генерация DOCX
- **papaparse 5.4.1** - парсинг CSV
- **Tabler Icons 3.26.0** - иконки

### **Backend**

**Сервер и API:**
- **Node.js v22.19.0** - серверная платформа
- **Express 5.1.0** - веб-фреймворк (порт 3001)
- **PostgreSQL (Render)** - основная база данных

**Аутентификация и безопасность:**
- **JWT (jsonwebtoken 9.0.2)** - токены доступа
- **bcrypt 5.1.1** - хеширование паролей
- **cookie-parser 1.4.7** - работа с cookies
- **cors 2.8.5** - CORS политики

**Дополнительные модули:**
- **Nodemailer 6.9.16** + **Resend** - отправка email
- **multer 1.4.5-lts.1** - загрузка файлов
- **swagger-jsdoc 6.2.8** - документация API

### **База данных**

**СУБД:** PostgreSQL на Render
- **Регион:** Frankfurt, Germany
- **Версия:** PostgreSQL 14+
- **Пул соединений:** pg 8.13.1
- **SSL:** включен (sslmode=require)

**Основные таблицы:**
```
users                    - пользователи
tenants                  - компании (тенанты)
user_tenants             - связь пользователей с компаниями
roles                    - роли (глобальные и тенантные)
permissions              - разрешения (23+)
user_role_assignments    - назначение ролей пользователям
role_permissions         - связь ролей и разрешений

projects                 - проекты
estimates                - сметы
estimate_items           - позиции смет
estimate_templates       - шаблоны смет

works                    - справочник работ
materials                - справочник материалов
work_materials           - связь работ и материалов
counterparties           - контрагенты

global_purchases         - общие закупки
purchases                - закупки по сметам
contracts                - договоры
work_completion_acts     - акты выполненных работ
schedules                - графики работ
```

---

## 🏗️ АРХИТЕКТУРА ПРОЕКТА

### **Структура папок**

```
vite/
├── app/                    # Frontend React приложение
│   ├── admin/             # Административные компоненты
│   │   ├── permissions/   # Управление разрешениями
│   │   └── users/         # Управление пользователями
│   ├── dashboard/         # Главная панель
│   ├── projects/          # Проекты и сметы
│   ├── references/        # Справочники (работы, материалы)
│   ├── purchases/         # Закупки
│   └── pages/            # Страницы (логин, регистрация)
│
├── server/                 # Backend Node.js
│   ├── controllers/       # Контроллеры API
│   ├── middleware/        # Middleware (auth, permissions)
│   ├── config/           # Конфигурация (БД, Swagger)
│   └── routes/           # Маршруты
│
├── api/                   # Vercel Serverless Functions
│   └── index.mjs         # Основная точка входа API
│
├── shared/                # Общий код
│   └── lib/              # API клиенты, утилиты
│
├── public/               # Статические файлы
│   └── favicon.png       # Иконка (используется как аватарка)
│
└── database-backups/     # Резервные копии БД
```

### **Архитектурные паттерны**

**Frontend:**
- Component-based architecture (React)
- Centralized state management (Redux)
- API layer abstraction (axiosInstance)
- Material Design (MUI)

**Backend:**
- RESTful API
- Controller-Service pattern
- Middleware-based auth & permissions
- Transaction-based DB operations

**Общее:**
- Multi-tenant SaaS architecture
- Role-Based Access Control (RBAC)
- JWT-based authentication
- Serverless deployment (Vercel)

---

## 🔐 СИСТЕМА БЕЗОПАСНОСТИ

### **Роли (3 типа)**

**Глобальные роли (tenant_id = NULL):**
1. **super_admin** - Супер-администратор
   - Полный доступ к системе
   - Управляет глобальными ролями
   - Не привязан к компании

2. **admin (template)** - Глобальный шаблон администратора
   - Копируется для каждой новой компании
   - Редактируется super_admin

**Тенантные роли (привязаны к компании):**
1. **admin** - Администратор компании
   - Управляет пользователями своей компании
   - Редактирует роли: manager, estimator, supplier

2. **manager** - Менеджер
   - Управляет проектами и сметами

3. **estimator** - Сметчик
   - Создает и редактирует сметы

4. **supplier** - Снабженец
   - Управляет закупками и материалами

### **Разрешения (23+)**

**Структура:** `resource.action`

**Родительские ресурсы:**
- **admin** - Администрирование (users, roles)
- **references** - Справочники (materials, works, counterparties)
- **projects** - Проекты (estimates, estimate_templates, purchases)

**Действия:**
- `view_menu` - видимость в меню
- `read` / `view` - просмотр
- `create` - создание
- `update` - редактирование
- `delete` - удаление

**Иерархия разрешений:**
```
admin.*
├── users.view_menu
├── users.read
├── users.create
├── users.update
└── users.delete

references.*
├── materials.view_menu
├── materials.read
├── works.view_menu
└── counterparties.view_menu

projects.*
├── estimates.read
├── purchases.view_menu
└── estimate_templates.view_menu
```

### **Middleware**

**authenticateToken** - проверка JWT токена
**checkPermission(resource, action)** - проверка разрешений с учётом иерархии

---

## 🚀 DEPLOYMENT

### **Production**

**Платформа:** Vercel
- **URL:** https://vite-pt47y0ymu-ilyas-projects-5a5f05a9.vercel.app
- **Vercel Token:** AzbO3kroECHEKi6JEnEFQgdj
- **Framework:** Vite
- **Build Command:** `npm run build`
- **Output Directory:** `dist`

**Serverless Functions:**
- Frontend: статические файлы из `dist/`
- Backend: `/api/index.mjs` (Express app)

**Environment Variables (.env.production):**
```env
DATABASE_URL=postgresql://[render_connection_string]
JWT_ACCESS_SECRET=your_super_secret_access_key_change_this_in_production_12345
JWT_REFRESH_SECRET=your_super_secret_refresh_key_change_this_in_production_67890
RESEND_API_KEY=re_2S3ZNHhd_9mCwZfkcVAD9Fmpq61fekM42
SENDER_EMAIL=noreply@smeta-lab.ru
FRONTEND_URL=https://smeta-lab.ru
```

### **Local Development**

**Запуск проекта:**
```powershell
# Terminal 1: Backend (порт 3001)
npm run server

# Terminal 2: Frontend (порт 5173)
npm run dev
```

**Скрипты package.json:**
```json
{
  "dev": "vite",
  "server": "node server/index.js",
  "build": "vite build",
  "preview": "vite preview"
}
```

### **Deploy команда**

```powershell
cd vite
vercel --prod --yes --token AzbO3kroECHEKi6JEnEFQgdj
```

---

## 👥 УЧЕТНЫЕ ЗАПИСИ

### **Super Admin**
- **Email:** kiy026@yandex.ru
- **Password:** !!!Apsni09332
- **Роль:** super_admin (глобальная)
- **Компания:** SMETA LAB

### **Tenant Admin**
- **Email:** i.sknewcity@gmail.com
- **Password:** !!!Apsni09332
- **Роль:** admin (тенантная)
- **Компания:** ООО СК Новый Город

### **Manager**
- **Email:** kuzminilya026@gmail.com
- **Password:** !!!Apsni09332
- **Роль:** manager
- **Компания:** ООО СК Новый Город

---

## 📊 КЛЮЧЕВЫЕ ОСОБЕННОСТИ РЕАЛИЗАЦИИ

### **1. Мультитенантность**
- Каждая компания (tenant) изолирована
- Пользователи могут быть в нескольких компаниях
- Роли и разрешения специфичны для каждой компании
- Данные фильтруются по `tenant_id`

### **2. JWT Аутентификация**
- Access Token (короткий срок) + Refresh Token (длинный срок)
- Auto-refresh при истечении access token
- В токене: `userId`, `email`, `tenantId`, `roles[]`, `permissions[]`

### **3. Иерархические разрешения**
- Родительское разрешение `admin.*` даёт доступ ко всем `admin.*` действиям
- Проверка разрешений в middleware через рекурсию
- Визуальная иерархия в UI управления разрешениями

### **4. Email Верификация**
- Отправка кода через Resend API
- Срок действия кода: 24 часа
- Автоматическая верификация для пользователей, созданных админом

### **5. Резервное копирование**
- Скрипт: `database-backups/create-backup.cjs`
- Формат: `smeta_full_backup_YYYY-MM-DDTHH-mm-ss.sql`
- Включает все таблицы и данные

---

## 🔧 ПОЛЕЗНЫЕ КОМАНДЫ

### **Работа с БД**

```powershell
# Проверка ролей пользователя
node vite/check-super-admin-roles.cjs

# Проверка глобальных ролей
node vite/check-global-roles.cjs

# Тест логики getAllRoles
node vite/test-get-all-roles.cjs

# Создание резервной копии
node database-backups/create-backup.cjs
```

### **Отладка**

```powershell
# Просмотр логов сервера
npm run server

# Проверка токена
node vite/decode-jwt-token.cjs

# Проверка разрешений пользователя
node vite/check-user-permissions.cjs
```

---

## 📚 ДОКУМЕНТАЦИЯ

- **API Swagger:** http://localhost:3001/api-docs
- **Permissions Reference:** `PERMISSIONS_REFERENCE.md`
- **Roles Architecture:** `ROLES_ARCHITECTURE.md`
- **Security Status:** `SECURITY_IMPLEMENTATION_STATUS.md`
- **Components List:** `COMPONENTS_LIST.md`

---

## 📝 ПОСЛЕДНИЕ ИЗМЕНЕНИЯ

**Deployment #117 (21.11.2025):**
1. ✅ Исправлена логика `rolesController.getAllRoles`
   - Было: проверка `roleKey === 'super_admin'` (неправильно)
   - Стало: проверка `userRoles.includes('super_admin')` (правильно)

2. ✅ Добавлена дефолтная аватарка `/favicon.png`
   - При регистрации нового пользователя
   - При создании пользователя администратором

3. ✅ Улучшено логирование во всех контроллерах
   - Детальные логи определения super_admin
   - Вывод списка ролей в консоль

**Известные проблемы:**
- Нет ограничений на размер загружаемых файлов
- Отсутствует rate limiting для API
- Email верификация не обязательна (можно улучшить UX)

---

## 🎯 ROADMAP

**Приоритет 1:**
- [ ] Система уведомлений (в приложении)
- [ ] Экспорт смет в Excel
- [ ] Шаблоны писем для email

**Приоритет 2:**
- [ ] Мобильная версия (responsive)
- [ ] Audit log (история действий)
- [ ] Двухфакторная аутентификация (2FA)

**Приоритет 3:**
- [ ] Интеграция с 1С
- [ ] API для внешних систем
- [ ] WebSocket для real-time обновлений

---

## 📞 КОНТАКТЫ

- **Production:** https://smeta-lab.ru
- **Vercel Dashboard:** https://vercel.com/ilyas-projects-5a5f05a9/vite
- **Backend:** Render Web Service
- **Database:** Render PostgreSQL (Frankfurt)
- **Email Support:** noreply@smeta-lab.ru

---

**Версия документа:** 1.1  
**Дата обновления:** 26 декабря 2025 г.  
**Автор:** GitHub Copilot + Development Team
