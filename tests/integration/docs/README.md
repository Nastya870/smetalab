# 🔗 Integration Tests

**Статус:** ✅ 26/26 passing (100%)

---

## 📋 Что тестируется?

### 1. Auth API (`tests/integration/api/auth.api.test.js`) - 18 тестов

#### POST /api/auth/register (5 тестов)
- ✅ Регистрация с валидными данными → 201
- ✅ Ошибка при невалидном email → 400
- ✅ Ошибка при слабом пароле → 400
- ✅ Ошибка при дублирующемся email → 409
- ✅ Ошибка при отсутствии обязательного поля → 400

#### POST /api/auth/login (5 тестов)
- ✅ Вход с правильными credentials → 200
- ✅ Ошибка при неверном пароле → 401
- ✅ Ошибка при несуществующем пользователе → 401
- ✅ Ошибка при невалидном формате email → 400
- ✅ JWT payload содержит роли и разрешения

#### POST /api/auth/refresh (3 теста)
- ✅ Обновление access token по refresh token → 200
- ✅ Ошибка при невалидном refresh token → 401
- ✅ Ошибка при отсутствии refresh token → 400

#### POST /api/auth/logout (2 теста)
- ✅ Успешный logout → 200
- ✅ Ошибка при отсутствии токена → 401

#### GET /api/auth/me (3 теста)
- ✅ Получение информации о текущем пользователе → 200
- ✅ Ошибка без токена → 401
- ✅ Ошибка с невалидным токеном → 403

---

### 2. Roles API (`tests/integration/api/roles.api.test.js`) - 8 тестов

#### GET /api/roles (4 теста)
- ✅ Super admin видит только глобальные роли
- ✅ Super admin определяется по ВСЕМ ролям пользователя
- ✅ Логирование корректной информации о super_admin
- ✅ Tenant admin видит только tenant roles своего тенанта

#### GET /api/permissions/roles/:id (1 тест)
- ✅ Получение полной информации о роли с разрешениями

#### Edge Cases (3 теста)
- ✅ Ошибка 401 без токена
- ✅ Ошибка 403 с невалидным токеном
- ✅ Ошибка 404 для несуществующей роли

---

## 🚀 Запуск

### Из корня проекта
```powershell
npm run test:integration
```

### Через скрипт
```powershell
.\test-scripts\integration-tests\scripts\run-integration.ps1
```

### Запуск отдельного файла
```powershell
npx vitest run tests/integration/api/auth.api.test.js
npx vitest run tests/integration/api/roles.api.test.js
```

### С детальным выводом
```powershell
npm run test:integration -- --reporter=verbose
```

---

## 🔧 Требования

### 1. Серверы должны быть запущены
```powershell
# Backend (в отдельном терминале)
npm run server
# → http://localhost:3001

# Frontend (в отдельном терминале)
npm run dev
# → http://localhost:5173
```

**Быстрый запуск обоих:**
```powershell
.\test-scripts\run-servers.ps1
```

### 2. База данных
- PostgreSQL (Render)
- Переменная DATABASE_URL в .env

### 3. Environment variables
```bash
DATABASE_URL=postgresql://...
JWT_SECRET=your-secret-key
```

---

## 🧹 Data Isolation

### Тестовые данные изолированы через email domains:
- **Auth tests:** `@test.com`
- **Roles tests:** `@rolestest.local`

### Cleanup выполняется автоматически:
```javascript
afterAll(async () => {
  await testDb.cleanupTestData();
  await testDb.closePool();
});
```

---

## 🐛 Troubleshooting

### Тесты падают с ошибкой "NO_TENANTS"
**Причина:** Тестовые данные не очистились после предыдущего запуска

**Решение:**
```powershell
node test-scripts/shared/utilities/list-users.mjs
# Найдите @test.com пользователей и удалите вручную через SQL
```

### Тесты падают с ошибкой FK constraint
**Причина:** Порядок удаления таблиц неправильный

**Решение:** Проверьте `testDatabase.js` — cleanup должен идти в обратном порядке FK зависимостей

### Ошибка "ECONNREFUSED"
**Причина:** Backend не запущен

**Решение:**
```powershell
npm run server
```

---

## 📊 Performance

**Среднее время выполнения:**
- Auth API tests: ~15-20s
- Roles API tests: ~10-12s
- **Total:** ~28-30s

---

## 🎯 План развития

1. ✅ Auth API (18 тестов)
2. ✅ Roles API (8 тестов)
3. ⏳ Works API (CRUD операции)
4. ⏳ Materials API (CRUD + import)
5. ⏳ Projects API (CRUD + estimates)
6. ⏳ Estimates API (расчёты, экспорт)
7. ⏳ Counterparties API
8. ⏳ Suppliers API

---

## 📖 См. также

- **[TESTING_GUIDE.md](../../TESTING_GUIDE.md)** — главное руководство
- **[README_TESTING.md](../../README_TESTING.md)** — детали сессии 21-22 ноября
- **[shared/fixtures/](../../shared/fixtures/)** — testDatabase.js
