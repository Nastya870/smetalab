...
export const createRole = catchAsync(async (req, res) => {
  ...
  // Валидация
  if (!key || !name) {
    throw new BadRequestError('Поля key и name обязательны');
  }
  ...
  if (existingRole.rows.length > 0) {
    throw new ConflictError('Роль с таким ключом уже существует в вашей компании');
  }
...
export const deleteRole = catchAsync(async (req, res) => {
  ...
  if (parseInt(usageCheck.rows[0].count) > 0) {
    throw new ConflictError('Невозможно удалить роль, так как она используется пользователями');
  }
